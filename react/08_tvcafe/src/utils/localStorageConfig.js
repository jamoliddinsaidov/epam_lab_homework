...
export const addUserCredentialsToLocalStorage = (values) => {
  ...
  let alreadyExists = users.some((user) => user.credentials.email === values.email)
...
export const loginUser = (values) => {
  ...
  let users = checkLocalStorageUsers()
  let user = users.filter((u) => u.credentials.email === values.email && u.credentials.password === values.password)
...
export const checkIsMovieFavorite = (id) => {
  let user = checkLocalStorageCurrentUser()
  let isFavorite = user.movies.favorites.filter((movie) => movie.id === id)[0]?.isFavorite
...
export const checkIsFollowed = (id) => {
  let user = checkLocalStorageCurrentUser()
  let isFollowed = user.friends.filter((friend) => friend.id === id)[0]?.isFollowed
