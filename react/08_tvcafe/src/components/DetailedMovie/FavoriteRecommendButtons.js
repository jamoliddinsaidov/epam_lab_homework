...
import { addFavoriteMovies, removeFromFavorites } from '../../utils/localStorageConfig'

const FavoriteRecommendButtons = ({ details, isRecommendClicked, setIsRecommendClicked }) => {
  // states
  const isFavoriteMovie = checkIsMovieFavorite(details.id)
  const [favoriteText, setFavoriteText] = useState(isFavoriteMovie ? 'Remove from favorites' : 'Add to favorites')
