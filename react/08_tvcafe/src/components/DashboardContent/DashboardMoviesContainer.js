...
const DashboardMoviesContainer = ({ user }) => {
  return (
    <StyledDashboardMoviesContainer>
      <DashboardMovieList movies={user.movies.favorites} title='Your Favorite Movies' key={user.id} />
      <DashboardMovieList movies={user.movies.recommended} title='Recommended Movies' key={user.email} />
    </StyledDashboardMoviesContainer>
  )
}
